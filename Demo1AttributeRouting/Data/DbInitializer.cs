...
namespace Demo1AttributeRouting.Data
{
    public static class DbInitializer
    {
        public static void Initialize(NorthWind northWind)
        {
            northWind.Database.EnsureCreated();

            if (!northWind.Authors.Any())
            {
                ...
                {
                    northWind.Authors.Add(c);
                }

                northWind.SaveChanges();
            }

            if (!northWind.Books.Any())
            {
                var books = new Book[]
                {
                    new Book()
